...
/**
 * Collapse all DOMRects from an anchor into one merged rect per visual line.
 * Multiple rects on the same line (e.g. from inline elements) are unioned together.
 *
 * @param anchor - The anchor span whose client rects should be normalised.
 * @returns One `DOMRect` per distinct visual line, sorted top-to-bottom.
 */
export function getNormalizedLineRects(anchor: HTMLElement): DOMRect[] {
